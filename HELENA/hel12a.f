...
C      CALL ROTATE_BND(FR,MFM,PAR1,IAS)
...
C------------------------------------------------------------------
     ...
     >                         (DPR(NINT+1)-DPR(NINT))
          ...
          IF (IPAI.EQ.7) THEN
	    FACTOR = 1.d0 + (FPI-1.d0)*(1.d0+TANH((PSI-EPI)/GPI))/2.d0
            DPDPSI = FACTOR * DPDPSI
	  ENDIF
